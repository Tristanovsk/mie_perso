...
C   any damages that may result from the use of the program. 
      Program main
! gfortran -o spher spher.f GAUSS.f
      IMPLICIT REAL*8 (A-H,O-Z)                                                 
      PARAMETER (NMIE=10000, NPL=2*NMIE)
      REAL*8 AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BET1(NPL),           
     &       BET2(NPL),MRR,MRI,LAM,nr,ni,nwater                          
      CHARACTER(len=1000)file_out,AAc,BBc,wlc,MRRc,MRIc
      ...
      NDISTR=5
      nwater=1.3199+6878/wl**2-1.132d9/wl**4+1.11d14/wl**6
      LAM=wl/nwater * 1d-3 !0.8634D0

      if(NDISTR==2)then
!with dispersion log distri
          NK=100
          N=100
          R1=1D-2
          R2=1D1
          sigma=0.0165d0*AA !46d0 !49d0
          sigma=sqrt(0.187) !45
    !beads 3microns; CV=1.%
          sigma=0.0075d0*AA !46d0 !49d0

    !beads 296nm; CV=1.8%
    !      sigma=0.018d0*AA !46d0 !49d0

    !dust %
          sigma=sqrt(0.55) !0.18d0*AA !46d0 !49d0
    ! formazin
    !      sigma=0.57
          BB=(sigma)**2D0
          write(AAc,'(f5.3)')AA
          write(BBc,'(f5.2)')BB
          file_out='../data/Mie_shchenko_ScatMat_log_rg'
     &             //trim(adjustl(AAc))//'_sg'//trim(adjustl(BBc))
      elseif(NDISTR==3)then
          NK=100
          N=100
          R1=1D-1
          R2=10D0
          sigma=.5d0 !*AA !46d0 !49d0
          BB=0.5 !(sigma)**2D0
      elseif(NDISTR==4)then      
!for theoretical monodisperse  
          NK=1
          N=1
          R1=AA*0.9999999 D0
          R2=AA*1.0000001 D0
          BB=1d-1
          write(AAc,'(f8.1)')2.*3.1415*AA/LAM
          file_out='../data/Mie_shchenko_ScatMat_x'//trim(adjustl(AAc))
      elseif(NDISTR==5)then
          NK=100
          N=100
          BB=-4
          R1=1D-2
          ...
          file_out='../data/Mie_shchenko_ScatMat_junge_slope'
     &             //trim(adjustl(BBc))//'_rmin'//trim(adjustl(AAc))
          write(AAc,'(f6.1)')R2
          file_out=trim(file_out)
     &             //'_rmax'//trim(adjustl(AAc))
      endif
...
!     nwater=1.329
!     if(wl<=425.)nwater=1.339
!     if(wl<=465.and.wl>425.)nwater=1.3375
!     if(wl<=500..and.wl>465.)nwater=1.3375
!     if(wl<=600..and.wl>500.)nwater=1.334
!     if(wl<=700..and.wl>600.)nwater=1.331
!     if(wl>700.)nwater=1.328
      ...
      file_out=trim(file_out)
     &         //'_nr'//trim(adjustl(MRRc))
     &         //'_ni'//trim(adjustl(MRIc))//'_wl'
     &         //trim(adjustl(wlc))//'.txt'
      open (11, file=trim(file_out))
      print*,'starting computations...'
      write(11,*)'### HEADER'
      write(11,*)' 1st line after header: Cext, Cscat'
      write(11,*)' 2nd line to EOF: Scat Angle (deg), Cosine,'
      write(11,*)' Gauss weight, F11, F33, F12, F34 '
      write(11,*)'### END OF HEADER'
...
C        180*(I-1)/(NPNA-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      PARAMETER (NMIE=10000, NPL=2*NMIE, NPNA=1801)
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 PI,A1(NPL),A2(NPL),A3(NPL),A4(NPL),B1(NPL),B2(NPL),
     &       cgaus(NPNA),pdgs(NPNA)
      PI=dacos(-1d0)
      L1MAX=LMAX+1
      D6=DSQRT(6D0)*0.25D0
      PRINT 1003
 1003 FORMAT(' ',5X,'<',11X,'F11',6X,'     F33',6X,'     F12',
     & 6X,'     F34')
      ...
      TB=-DB
      TAA=-DA
!_______call gauss angles for scat matrix reconstruction
      !call GAUSS(N,-1.,1.,cgaus,pdgs )! 
      cgaus(N)=1.0
      pdgs(N)=0.0
       call gauss_ext(-1.d0,1.d0,cgaus,pdgs,N-2) 
      do j=1,N-2
       cgaus(N-j)=cgaus(N-j-1)
       pdgs(N-j)=pdgs(N-j-1)
      enddo
      cgaus(1)=-1.0
      pdgs(1)=0.0
!      open(4,file='/home/harmel/OSOA_harmel/BASE/mu40')
!        do j=1,N 
!        read(4,222) cgaus(j),pdgs(j)
!!        print*,dacos(xmu(j))*180./pi,xhr(j)
!        enddo
! 222  format(2d21.14)
!      CLOSE(4)
      ...
      DO 500 I1=1,N
         ...
         !TB=TB+DB
         TAA=dacos(cgaus(I1))
         TB=180./pi*dacos(cgaus(I1)) !TB+DB
         U=DCOS(TAA)
         F11=0D0
         F2=0D0
         F3=0D0
...
C        F34=100d0*F34/F11
         ...
         WRITE (11,1004) TB,cgaus(I1),pdgs(I1),F11,F33,F12,F34
  500 CONTINUE
 1004 FORMAT(' ',F6.2,6(x,E16.9))
      RETURN
      END
